...
$sql = "SELECT * FROM subject WHERE teacher='$teacher' GROUP BY grade_level, section";
$result = $conn->query($sql);
...
if ($result->num_rows > 0) {
    while ($row = $result->fetch_assoc()) {
        ...
        $section = $row['section'];
        $subject = $row['subject']; // Ensure $subject is defined (added for safety)

        // Modified SQL to sort by lastname, then firstname
        $sql2 = "SELECT * FROM student 
                 WHERE grade_level='$grade_level' 
                 AND section='$section' 
                 AND sy='$sy' 
                 GROUP BY id 
                 ORDER BY lastname ASC, firstname ASC"; // Sort alphabetically
        ...
        if ($result2->num_rows > 0) {
            while ($row2 = $result2->fetch_assoc()) {
                $sid = $row2['id'];
                echo '<a class="affan-element-item" href="teacher_view_all_grades.php?section='.$section.'&grade_level='.$grade_level.'&subject='.$subject.'&sid='.$sid.'">
                        <text> [ <text class="text-success"><i class="fa-regular fa-id-badge"></i> '.$row2['id'].'</text> ] 
                        '.$row2['lastname'].', '.$row2['firstname'].' - Grade '.$row2['grade_level'].' '.$row2['section'].'  
                        </text> 
