...
if (isset($_POST["submit"])) {
    ...
    if (isset($_FILES['file']['tmp_name']) && $_FILES['file']['tmp_name']) {
        ...
        while (($filesop = fgetcsv($handle, 1000, ",")) !== false) {
            ...
            $grade_level = $filesop[2];
            $time = $filesop[3];
            $section = $filesop[4];
            $teacher = $filesop[5];
            
            // Prepare and execute the SQL statement
        $sql = "INSERT INTO subject (subject, description, grade_level,time, section, teacher) 
        VALUES (?, ?, ?, ?, ?,?)";
$stmt = mysqli_prepare($conn, $sql);
mysqli_stmt_bind_param($stmt, 'ssssss', $subject, $description, $grade_level, $time, $section, $teacher);
